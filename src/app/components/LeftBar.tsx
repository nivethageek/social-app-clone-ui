...
function LeftBar() {
    return (
        <div className='h-screen sticky top-0 flex flex-col justify-between pt-2 pb-8'>
            ...
            <Image src={`icons/${item.icon}`} alt='leftBar Image' width={24} height={24}/>
            <span className='hidden xxl:inline'>{item.name}</span>
             ...
             <div className='w-10 h-10 relative rounded-full overflow-hidden'>
                <Image src='/general/avatar.png' alt='shareWorldz' fill/>
             </div>
             <div className='hidden xxl:flex flex-col'>
                <span className='font-bold'>ShareWorldz</span>
                <span className='text-sm text-textGray'>@shareWorldz</span>
             </div>
             </div>
             <div className='hidden xxl:block cursor-pointer font-block'>...</div>
             </div>
        </div>
    );
}
